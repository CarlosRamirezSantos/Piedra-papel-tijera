...
function connectSocket() {
    ...
    socket.on('resultado-ronda', (data) => {
        ...
        let texto = `Tú jugaste: <b>${obtenerMiJugada(data)}</b> vs Rival.`;
        texto += `<br>Ganador ronda: <b>${data.ganadorRonda}</b>`;
...
if(btnCreateGame) {
    btnCreateGame.addEventListener('click', () => {
        console.log("👉 CLICK EN CREAR PARTIDA"); // <--- AÑADE ESTO
...
document.querySelectorAll('.move-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
        ...
        if(roundMessage) roundMessage.textContent = `Elegiste ${jugada}. Esperando resultado...`;
    });
});
...
if(btnAbandon) {
    btnAbandon.addEventListener('click', () => {
        if(confirm("¿Salir de la partida?")) window.location.reload();
    });
}
